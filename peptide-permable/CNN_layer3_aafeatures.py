...
'''
[[0.9355390306094532, 0.88901897018970177], [0.93722982807489863, 0.89461788617886173], [0.93941309082154156, 0.89867750677506764]]
0.91847696477 --> WITHOUT dROPOUT
0.914514905149 --> dropout
'''
RESULT = {}
import sys
if len(sys.argv)==2:
...
for repeat in range(0,1): #perform 5 repeats
    ...
    ##                                     int(100*np.mean(best_roc_val[1])),int(100*np.mean(best_roc_val[2]))))
            ...
            if len(best_logit_test) >=3 and best_logit_test[0][1] <= roc_val:
                model_name = 'model4_%s_%s_%s_%s_%s_%s.ckpt' %(test,CV,repeat,str(roc_train)[:5],str(roc_val)[:5],str(roc_test)[:5])
