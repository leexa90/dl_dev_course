        ...
        for j in all_models: #find if model they belong to that set
            if j[7] == str(i1) and  j[9] == str(i2) :
                if (i1,i2) not in dictt_model:
                    ...
                    dictt_model[(i1,i2)] += [j,]
for c1 in range(0,12): #first three res #start from4
    for c2 in range(c1+1,12):
        for c3 in range(c2+1,12):
             if c2 not in unchanged and c1 not in unchanged and c3 not in unchanged: #readable but slower
                ...
                data= pd.DataFrame(Inp0_,columns=range(12))
                data['prob']  = 0
                counter =0 
                ...
                data.to_csv('results/resultsPMI_%s_%s_%s.csv' %(c1,c2,c3),index=0)
