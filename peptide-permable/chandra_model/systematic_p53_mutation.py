...
def get_data_from_X(X,i=i,num=8000): #get tensor inputs from X and y
    Inp0_ = np.array([x[0] for x in X])
    Inp1_ = np.array([x[1] for x in X])
    Inp2_ = np.array([x[2] for x in X])
    labels_ = np.nan
    return Inp0_,Inp1_,Inp2_,labels_
