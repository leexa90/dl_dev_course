...
repeat = 0
xgb_features = ['per_A', 'num_A', 'per_C', 'num_C', 'per_E', 'num_E', 'per_D', 'num_D',
                'per_G', 'num_G', 'per_F', 'num_F', 'per_I', 'num_I', 'per_H', 'num_H',
                'per_K', 'num_K', 'per_M', 'num_M', 'per_L', 'num_L', 'per_N', 'num_N',
                'per_Q', 'num_Q', 'per_P', 'num_P', 'per_S', 'num_S', 'per_R', 'num_R',
                'per_T', 'num_T', 'per_W', 'num_W', 'per_V', 'num_V', 'per_Y', 'num_Y',
                'length', 'netcharge', 'Gwif', 'Goct']
data['preb_xgb'] = 0
for test in range(0,5):
    print "TEST IS",test
    test_emsemble= []
    X = data['X'].copy(deep=True)
    X_xgb = data[xgb_features].copy().values
    X_test = []
    X_xgb_test = []
    y_test = []
        ...
        for CV in range(folds-1): #for each repeat, do 4 fold CV. (test set is kept constant throughtout)# 
            ...
            if True:
                ...
                for file in  sorted(dictt_model[(test,CV)],
                    ...
                    best_roc_val[file] = [roc_train,roc_val,roc_test,logit_test]
            ...
            xgtest    = xgb.DMatrix(X_xgb_test, label=None,missing=np.NAN,feature_names=xgb_features)
            xgb_temp = []
            for file in [x for x in os.listdir('../') if ('XGB1_%s'%test in x and '.ckpt' in x)]:
                bst = xgb.Booster({'nthread':1})
                bst.load_model('../'+file)
                xgb_temp +=  [bst.predict(xgtest),]
            zz = data.set_value(range(test,len(data),5),'pred_xgb',np.mean(xgb_temp,0))

            
#data[['seq','source','pred']].iloc[range(test,len(data),5)].to_csv('model7.csv',index=0)
