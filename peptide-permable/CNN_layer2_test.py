...
with tf.name_scope('inputs') as scope:
    ...
    dropout = 0.5#tf.placeholder(tf.float32,name='dropout')
...
with tf.name_scope('layer1') as scope:
    ...
    layer1_DO = tf.layers.dropout(layer1,rate=dropout,training=True,name='Drop1')
        ...
        for epoch in range(training_epochs):#training_epochs):
            ...
            for i in shuffle[::10]: #training with bagging
                ...
                _, c = sess.run([optimizer, acc], feed_dict={Inp0: Inp0_,Inp2: Inp2_,
                                                                   Inp1: Inp1_,
                                                                   labels: labels_,#dropout : 0.1,#
                                                                   learning_rate : lr}) #sgd
            die
            logit_train = []
            cost_train = []
            lr = 0
