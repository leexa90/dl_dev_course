...
data = data.merge(data_xgb[['testPred','seq','source']],on=['seq','source'])
features += ['testPred',]
...
with tf.name_scope('inputs') as scope:
    ...
    Inp2 = tf.placeholder(tf.float32, [batch_size,45] ,name = 'globa_seq_info')
...
with tf.name_scope('dense') as scope:
    ...
    layer5_DO = tf.layers.dropout(gbmp_extra,rate=0,name='Drop5',training=True)# there are some important info here, cannot dropout
...
0.937913279133 - > 512mean + 512max , dropout 0.1, maxpool
- > 512mean + 512max , avg pool
'''
...
for repeat in range(0,1): #perform 5 repeats
    for CV in range(folds-1): #for each repeat, do 4 fold CV. (test set is kept constant throughtout)# 
        ...
        for epoch in range(training_epochs):#training_epochs):
            ...
            for i in shuffle[::10]: #training with bagging
                lr = 0.0025*(1-1.0*counter/len(shuffle[::10]))
                counter += 1
    ...
    ##                                     int(100*np.mean(best_roc_val[1])),int(100*np.mean(best_roc_val[2]))))
            if roc_val <= 0.5: # if val worst then chance, reinitalize tarining
                init = tf.global_variables_initializer();sess = tf.Session();sess.run(init)
            best_logit_test = sorted([best_roc_val[ep] for ep in best_roc_val], key = lambda x :x[1])[-4:]
            if len(best_logit_test) >=4 and best_logit_test[0][1] <= roc_val:
