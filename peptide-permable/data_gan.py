...
#    Inp2 = tf.reduce_mean(Inp2_a,axis=1)
    Inp2 = tf.placeholder(tf.float32, [None,4] ,name = 'globa_seq_info') # converted to embeddings
...
with tf.name_scope('dense') as scope:
    ...
    dense1 = tf.layers.dense(layer5_DO,16,activation = tf.nn.relu , name = 'dense1' )
    layer6_DO = tf.layers.dropout(dense1,rate=dropout,name='Drop6')
    dense2 = tf.layers.dense(layer6_DO,16 , name = 'dense2' )
    dense3 = tf.layers.dense(globalmaxpooling,1 , name = 'dense3' )
      ...
      if CV != test:
        ...
        for epoch in range(training_epochs):#training_epochs):
            ...
            for i in range(len(X_train)):
                ...
                out = sess.run( out_softmax, feed_dict={Inp0: Inp0_,Inp2: Inp2_,
                                                      Inp1: Inp1_,
                                                      labels: labels_,
            ...
            print np.mean(cost_train),np.mean(cost_val),np.mean(cost_test)
