...
import os
all_models = [x[:-5] for x in os.listdir('.') if ('.ckpt.meta' in x and x.startswith('CNN_layer2_test.pyLSTM1'))]
dictt_model = {}
for i1 in range(0,5): #test set
    for i2 in range(0,4): #CV set
        for j in all_models: #find if model they belong to that set
            if j[24] == str(i1) and  j[26] == str(i2) :
...
import os
all_models = [x[:-5] for x in os.listdir('.') if ('.ckpt.meta' in x and x.startswith('CNN'))]
dictt_model = {}
for i1 in range(0,5): #test set
    for i2 in range(0,4): #CV set
        for j in all_models: #find if model they belong to that set
                    ...
                    if True:
                        for test_fold in dictt_model:
                            for file in  sorted(dictt_model[test_fold],
                                ...
                                length_ = np.array([13,]*80000)
                                b = sess.run (out_softmax, feed_dict={Inp0: Inp0_,sequence_length : length_,
                                                                                Inp1: Inp1_,Inp2: Inp2_,
                                                                                dropout : 0,learning_rate : 0})
                                thres = 0.7
                                data['prob'] = np.reshape(b,(80000)) + data['prob']
                                data['fold'+str(file) ] = b
                                counter += 1
                        data['prob'] =  data['prob'] /counter
                        data.to_csv('results_80K10_%s_%s_%s.csv' %(c1,c2,c3),index=0)
...
for test in range(0,5):
    temp_xgb = []
    X = data_ori.iloc[test::5][xgb_features].copy().values
    xgtest    = xgb.DMatrix(X, feature_names=xgb_features)
    for file in [x for x in os.listdir('../') if ('XGB1_%s'%test in x and '.ckpt' in x)]:
        bst = xgb.Booster({'nthread':6})
        bst.load_model('../'+file)
        temp_xgb += [bst.predict(xgtest),]
    data_ori.set_value(range(test,len(data_ori),5),'prob', np.mean(temp_xgb,0))
